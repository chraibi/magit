...
with the variable values as arguments that were recorded by
...
(cl-defmethod magit-bookmark-name (&context (major-mode magit-diff-mode))
  (format "magit-diff(%s%s)"
          (pcase (magit-diff-type)
            ...
            (`undefined (cons magit-buffer-typearg magit-buffer-range-hashed)))
...
(cl-defmethod magit-bookmark-name (&context (major-mode magit-log-mode))
  (format "magit-log(%s%s)"
          (mapconcat #'identity magit-buffer-revision " ")
...
;; TODO (put 'magit-submodules-mode 'magit-bookmark-variables nil)
;; This derives from `tabulated-list-mode', not `magit-mode'.
